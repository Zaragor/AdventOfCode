...
if __name__ == "__main__":
    ...
    wire_one_pos = list_positions(wireOne)
    wire_two_pos = list_positions(wireTwo)
    wire_one_pos_set = set(wire_one_pos)
    wire_two_pos_set = set(wire_two_pos)

    intersections = (wire_one_pos_set.intersection(wire_two_pos_set))
    def wire_length(intersection):
            wire_one_length = wire_one_pos.index(intersection)
            wire_two_length = wire_two_pos.index(intersection)
            return wire_one_length + wire_two_length
    length_list = list(map(wire_length, intersections))
    length_list.sort()
    print(length_list)

    for intersection in intersections:
        print("{} - {}".format(intersection, abs(intersection[0]) + abs(intersection[1])))